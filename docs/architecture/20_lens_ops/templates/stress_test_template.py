...
def stress_test_list(jwt_token: str, concurrency: int) -> List[Dict[str, Any]]:
    ...
    """
    print(f"\nStress Testing: GET /v1/actions/list?domain={DOMAIN}")
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        futures = [
            executor.submit(make_request, jwt_token, f"/v1/actions/list?domain={DOMAIN}")
...
def stress_test_execute(jwt_token: str, concurrency: int) -> List[Dict[str, Any]]:
    ...
    payload = {
        "action": "{READ_ACTION}",  # e.g., "view_fault_detail"
        "context": {"yacht_id": "{YACHT_ID}"},
        "payload": {
            "yacht_id": "{YACHT_ID}",
            "{ENTITY_ID_KEY}": "00000000-0000-0000-0000-000000000000"  # Fake ID
        }
    }
