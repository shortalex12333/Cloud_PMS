...
## Comments ## 
Do you beleieve that faults are differnet tbale/lsit to work orders? any crew cna raise a fault, adn terefore only that specific department can riase and create work order based upon this? therefore ntofiication sent to engineering department of the fault #1, will be visible in ledger, prompting suers to "create work order"? or see related equipment? lets assuem crew are non tehcnical, they raise "fan makign funnny nosie", we need to direct them through drop down adn vairabels know to then help engineers link to manuals, aprts etc. otherwose, non-tehcnical crew inserts "fan making weird noise in crew area, main dekc aft". this could be really understnabale to veteran crew, but for new, it is impossible. hwere do you find manual? how can celeste link to manual etc?
we are going to cinoporate a future project of "show related" visible when users opena n entity card (wo, manual, hdnover etc.). this is called "show related", and like the name syas, allwos crew to quickyl scroll to see the realted ifnromationr egarding data in question. see below for description; “Show Related” side page (what it is, what it’s not)

What it is: a side panel (not a new page, not navigation) that runs a deterministic “relatedness” retrieval for the currently focused entity (WO, equipment, fault, part, etc.). It pulls the most relevant known operational context: parts, manuals, handovers, notes, prior WOs, attachments.

What it’s not: global search, a dashboard, or a second workflow. It doesn’t invent actions. It doesn’t guess “urgency.” It doesn’t change the user’s journey. It’s an “expand context” panel.

Canonical user story (your example)

Sarah opens: “MTU Main Engine Overhaul” work order (focused entity = work_order:<uuid>)

She clicks Show Related and the panel returns:

Parts used / required for that WO + parts historically used on this equipment/system

Official manual(s) (e.g., MTU ME manual, relevant section)

Previous handover notes mentioning this engine / overhaul / same parts

Prior work orders on the same equipment + same system tags

Evidence files (photos, PDFs) attached to relevant WOs/faults

All results are filtered by RLS + role scope. If she shouldn’t see it, it doesn’t appear.

How “Show Related” retrieves (deterministic, not vibes)
Inputs (only from the focused entity + DB facts)

When user clicks the button, backend builds a “related query object”:

Focused entity:

entity_type, entity_id, yacht_id

Entity facts (DB):
For a WO:

work_order.title, description, equipment_id, fault_id

type, priority, status

metadata fields if present (system tags, model, vendor, etc.)

linked tables (parts lines, attachments, notes)

Entity-derived tokens (controlled):

equipment name/model/serial (if exists)

system name (if exists)

part_number/manufacturer (if exists)

canonicalized keywords from title/description (strict extraction)

Retrieval sources (ranked buckets)

The panel queries in layers, merging results:

FK joins (hard truth, highest priority)
...
Similarity search (vector) using entity fields only

Search text is built from known fields (not user query)

e.g. embedding_text = "MTU main engine overhaul | equipment: MTU 16V2000 | system: propulsion | part_numbers: ..."

Heuristic boosting (deterministic)

Same equipment_id gets boosted

Same system tag gets boosted

Same manufacturer + part_number gets boosted

Recent handovers boosted slightly

No “RAG reasoning” needed. It’s retrieval + ranking.

What the UI shows (side panel structure)

Panel header: “Related to: MTU Main Engine Overhaul (WO-####)”

Sections (collapsible, consistent order)
...
why it matched (simple, deterministic reason chips like “same equipment”, “mentions part ENG-0198-824”, “linked via FK”)

one-click “Open” (focus that entity; doesn’t navigate away)

Bottom action: Add related items button

“Add related items” (user-driven, uses global search)

This is important: users can curate context.

Flow:

user clicks Add related items

you open the global search bar in a “selection mode”

user types keywords/terms or searches for an existing doc/WO/handover

results list appears (already filtered by role/RLS)

user selects one or more items

user confirms “Attach as related”

backend writes relationship rows (deterministic)

Relationship storage (don’t overcomplicate)

You need one table like:
...
link_type (e.g., related, reference, evidence, manual)

created_by, created_at

optional note

This is the “known facts of pairing” you mentioned. It becomes the strongest signal for future related panels.

Permissions / scope rules (how results get filtered)

Two layers always apply:

DB RLS: if the row isn’t visible, it can’t be returned.

Action gating: if an entity is visible but an action isn’t allowed, actions don’t appear.

For “Show Related” specifically:

It’s read-only retrieval. It returns entities the user can already SELECT.

If a user can’t view documents bucket (read-only restrictions), they’ll still see doc metadata only if allowed, and file open/download is blocked by Storage policy.

Output contract (what backend returns)

Return a grouped payload like:

focused_entity

groups[] each with type, items[]
...
match_reasons[] (deterministic strings)

open_action (focus entity)

add_related_enabled boolean

missing_signals[] (optional debug: “no equipment_id on WO, FK linking limited”)

No extra actions here unless user intent is explicit.

Key guardrails (to prevent drift into “second search engine”)

Show Related uses entity fields, not the user’s last query text.

No actions on the related list unless user explicitly asked for an action.

No “critical” theatre. If severity exists, it’s metadata and affects notifications/filters, not banners.

Everything is auditable when the user adds a link (write event).
what this means for us, is the faualt information eneds to be imfortance, legible, auto popualted in areas, and editable by further tehcnical crew once viweing. to therefore link correctly,a dn allow us to change related docs.

- "create work order from this fault" there is no drop down of "faults" however you raise a good conern, potentially we are at our limit to how far a searhc abr can go. without fault list clickable, tehcnical crew would sturggle to see. therefore we would need to incporpoate this. 
further more, this means that the query "create work order from this fault" is impossible. a lsit of faults is nto searchable nor querable. therefore the micraciton would have to be present ONCE tehnical crew click on fault accordingly. along with array of other actions possible as triggers are minimal, we can never assume. 

- it is to be ntoed, when crew ar emakign their handover. the most critial peices of information are breakdown maintenance. not necessarily planned. therefore "add to hadnvoer" button present will be exciting to crew. as we have a process for auto-creating these hadnovers in professional format. you need to think about when naturally these can appear. liekly after creating work order, comeptlting, draft or inprogress, archived, crew cna add to their hadnover. potnetially we notify once fault w/o marked as compelte to push to handover? otherwise what other triggers could naturally be selected.? creating work order for eahc fault is rgeat focus. doucments what was used, how, parts, time , whom, etc. therefore all data in *this* work order would be higley avaluebal for handover creation.

- "close bilge pump fault" excellent query that could happen. if we have a lsit, this is good. but users can also query. mark hwo this is defined, guard rails, never assume suers interaciton with querying is non-chatoic. users are disoragnised, mispell and illiterate. never ASSUME but prepare for difference in laniage temrinilgoya dh hwo to surface multiple optiosn. we will NEVER surface just 1 restuls form searhc, as nautarllly each query entity term type and vraibale will coincide and pair with **SOME** value in our db. but this liekly means we need to evaualte the tnity exytraction pipeline, adn prepapre module, sql rag, and microactiona tatchment/psuh accordingly. plan methodically.
- you ahve not mentioned warranty claims in your scenarios. users may reqest any lanauge to start a warranty, attaching a photo either prior or after starting warranty. linking to part, manual. and how we can mark as "warranty claims". we will need new buclet creating for this. and finailsie this procedure with "write na email". we already aheva "write" email app onazure, adn have email funciton on site. but we will never auto-send. only auto popualte with iamge/upload from user, equipemnt detials, note etc. however all these detiasl for email will ALREADY be entered by suers when makign warranty claim. all we do is articualte ot bbetter english. this is final projhect that will be shleved. simple workflwo to pass through detials and get gpt4o-mini to paraphrase and write as email with NO signature and no user, then input into body of emial, adn allow users to preview, edit thn send to accordingly to sender via their address book, adn CC in required parties onboard etc.
otherwise lower rank crew will be able to forward to their HoD. only HoD will have ability to forward claim via email to suppliers. not lower rank.