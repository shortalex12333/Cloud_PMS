...
5. Verify current FE-Phase (from GSD_PHASE_PLAN.md) before touching code
   NOTE: FE-Phases are SEPARATE from ROADMAP.md Phases 1–13. Do not confuse them.
...
### ALWAYS
- Run CODEBASE_AUDIT.md scans before FE-Phase 0. Hardcoded styles silently override tokens.
- Before creating ANY new component: `find src/ -iname "*ComponentName*"` to check for existing duplicates.
- After creating ANY new component: verify only ONE version exists in the codebase.
- Import from barrel files (`@/components/ui`), never direct file paths.
