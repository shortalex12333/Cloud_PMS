...
| File | What it does + when you'd touch it |
|---|---|
| `action_execution_routes.py` | Receives every button-click action from the frontend. The single entry point for all actions. After every successful mutation it: (1) writes a ledger event via the Phase-B safety net (using `action_router/ledger_metadata.py`), and (2) calls `enqueue_for_projection()` to re-index the entity in `search_index`. *Touch this if an action is returning 404 or 500 before even reaching the handler — the routing itself is broken. If a successful mutation never appears in search, check that the action is in `ledger_metadata.py`.* **CRITICAL:** Contains `_WORK_ORDER_ACTIONS` frozenset (line ~267) and `resolve_entity_context()` function. If a work-order action returns **400 "work_order_id is required"**, the action name is missing from `_WORK_ORDER_ACTIONS` — `resolve_entity_context()` only maps `context.entity_id` → `work_order_id` for actions in that set. Add the action name to the frozenset. |
...
| `entityRoutes.ts` | Maps entity types to URLs. *e.g. clicking an equipment link goes to the wrong page → the route mapping is here.* **RULE:** `getEntityRoute()` requires SINGULAR entity types: `'part'`, `'fault'`, `'document'`, `'equipment'`, `'work_order'`. Passing a plural (`'parts'`, `'faults'`, `'documents'`) silently returns `'/'` (homepage) because the type is not in `routeMap`. Never pass plural strings to this function. |
