...
| 2026-04-23 | **PR #652 — 422 fix + formatter.** Bug 1: `vessel_surface_routes.py:672` raised `le=500`, frontend requested `limit=1000` → FastAPI hard-rejected with 422. Fixed `le=2000`. Bug 2: `_format_record()` had no `elif domain == "documents":` case — all docs showed "Untitled". Added formatter reading `filename`, `doc_type`, `storage_path`, `size_bytes`, `content_type`, `tags`. Bug 3: `_name_column()` had `"documents": "name"` but column is `filename`. Fixed. Bug 4: `_format_inline_result()` also referenced `"name"` and `"document_type"` (wrong column names). Fixed to `filename` and `doc_type`. All changes in `vessel_surface_routes.py`. Full detail: `BUGFIX_SESSION_2026-04-23.md` §Bug 1 + §Bug 2. | DOCUMENTS04 |
| 2026-04-23 | **PR #654 — Folder tree + PDF viewer + PO action routing.** Bug 3: `docTreeBuilder.ts:142` was reading `original_path` (upload-machine filesystem path) to build folder tree — showed `C:\Users\celeste7\Documents\...` instead of bucket structure. Fixed to `storage_path` with UUID prefix stripping. Bug 4: `DocumentContent.tsx` had no viewer — only a static title placeholder. `loadDocumentWithBackend()` existed in `documentLoader.ts` but was never imported. Added `POST /v1/documents/{doc_id}/sign` endpoint to `document_routes.py` (10-min signed URL, yacht-scoped). Wired `DocumentContent.tsx` to call it on mount, added `<iframe>`/`<img>` viewer with CSS spinner. Also fixed 6 PO action routing bugs in same commit (see `BUGFIX_SESSION_2026-04-23.md` §Bug 5-10). Both PRs merged to main. Render + Vercel deployed. | DOCUMENTS04 |
