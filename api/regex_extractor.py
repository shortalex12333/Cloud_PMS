...
# Lazy-load spaCy (CRITICAL: Prevents segfault with Gunicorn preload_app=True)
# spaCy cannot be loaded at module import time when using fork()-based servers
# Load it lazily in each worker process instead
_nlp = None  # Will be loaded on first use
_spacy_available = None  # Cache availability check

def _get_spacy():
    """
    Lazy-load spaCy NLP model with maritime domain enhancements.
    IMPORTANT: Do NOT load spaCy at module level when using Gunicorn with preload_app=True.
    spaCy's C extensions don't handle fork() properly and cause SIGSEGV crashes.

    Enhancements:
    - Phase 2: Custom Maritime NER model (91% F1 score, 10 entity types)
    - Phase 1: Entity Ruler with 286 maritime patterns (fallback)
    - Custom entity types: CREW_ROLE, VESSEL_LOCATION, VESSEL_EQUIPMENT, VESSEL_SYSTEM, MARITIME_DOC,
                          MAINTENANCE_ACTION, OPERATIONAL_STATE, SYMPTOM, ORG, MEASUREMENT
    """
    global _nlp, _spacy_available

    # Return cached model if already loaded in this worker
    if _nlp is not None:
        return _nlp

    # Return None if we already know spaCy is unavailable
    if _spacy_available is False:
        return None

    # Try to load spaCy for the first time in this worker
    try:
        import spacy
        from pathlib import Path
        try:
            from api.maritime_spacy_enhancements import enhance_spacy_with_entity_ruler
        except ImportError:
            from maritime_spacy_enhancements import enhance_spacy_with_entity_ruler

        # Try to load custom maritime NER model (Phase 3: with Word2Vec embeddings)
        custom_model_v2_path = Path(__file__).parent / "models" / "maritime_ner_v2"
        custom_model_v1_path = Path(__file__).parent / "models" / "maritime_ner_v1"

        if custom_model_v2_path.exists():
            # Load Phase 3 model with Word2Vec embeddings
            _nlp = spacy.load(str(custom_model_v2_path))
            print(f"[MARITIME NER] ✅ Loaded custom model: maritime_ner_v2 (Phase 3 - with Word2Vec)")
            print(f"[MARITIME NER] F1: 91.05% | Embeddings: 16,112 words, 300-dim")
            print(f"[MARITIME NER] Entity types: CREW_ROLE, VESSEL_LOCATION, VESSEL_EQUIPMENT, VESSEL_SYSTEM,")
            print(f"[MARITIME NER]               MARITIME_DOC, MAINTENANCE_ACTION, OPERATIONAL_STATE, SYMPTOM, ORG, MEASUREMENT")
        elif custom_model_v1_path.exists():
            # Fallback to Phase 2 model (without embeddings)
            _nlp = spacy.load(str(custom_model_v1_path))
            print(f"[MARITIME NER] ✅ Loaded custom model: maritime_ner_v1 (Phase 2 - no embeddings)")
            print(f"[MARITIME NER] F1: 91.05%")
            print(f"[MARITIME NER] Entity types: CREW_ROLE, VESSEL_LOCATION, VESSEL_EQUIPMENT, VESSEL_SYSTEM,")
            print(f"[MARITIME NER]               MARITIME_DOC, MAINTENANCE_ACTION, OPERATIONAL_STATE, SYMPTOM, ORG, MEASUREMENT")
        else:
            # Fall back to base model + Entity Ruler (Phase 1)
            print(f"[MARITIME NER] ⚠️  Custom models not found")
            print(f"[MARITIME NER] Falling back to base model + Entity Ruler (Phase 1)")
            _nlp = spacy.load("en_core_web_sm")
            _nlp = enhance_spacy_with_entity_ruler(_nlp)
        ...
        return None  # Gracefully degrade if spaCy not available
    except OSError as e:
        print(f"[ERROR] spaCy model not found: {e}")
        _spacy_available = False
        return None  # Model not installed
    except Exception as e:
        print(f"[ERROR] Unexpected error loading spaCy: {e}")
        _spacy_available = False
        return None
...
class RegexExtractor:
    ...
    def extract(self, text: str) -> Tuple[List[Entity], List[Tuple[int, int]]]:
        ...
        # Apply spaCy NER and noun extraction (NEW - captures proper nouns, generic terms, abstract entities)
        nlp = _get_spacy()
        if nlp is not None:
            spacy_entities, spacy_spans = self._spacy_extract(text, extracted_texts, extracted_spans, nlp)
            entities.extend(spacy_entities)
            covered_spans.extend(spacy_spans)
    ...
    def _spacy_extract(self, text: str, already_extracted: Set[str], existing_spans: List[Tuple[int, int]], nlp) -> Tuple[List[Entity], List[Tuple[int, int]]]:
        """
        Extract entities using spaCy NER and noun extraction.

        Captures:
        - Named entities (ORG, PERSON, PRODUCT) → org
        - Nouns (yacht, crane, motor) → equipment
        - Abstract nouns (activation, payment, service) → action
        - Noun chunks (multi-word terms) → appropriate type

        Args:
            text: Input text
            already_extracted: Set of already extracted terms (lowercase)
            existing_spans: List of already covered spans
            nlp: spaCy language model (loaded lazily)

        Returns:
            (entities, spans) tuple
        """
        entities = []
        spans = []
        ...
        # --- Part 1: Named Entity Recognition (NER + Custom Maritime Model) ---
        # Extracts:
        # - Phase 2: Custom Maritime NER (10 entity types, 91% F1)
        # - Phase 1: Entity Ruler (286 patterns, fallback)
        # - Generic NER: proper nouns like "Gabriel Edward Jewelers", "Microsoft"
        for ent in doc.ents:
            # Map spaCy entity labels to our internal types
            entity_type = None

            # Phase 2: Custom Maritime NER labels (10 types)
            if ent.label_ == 'CREW_ROLE':
                entity_type = 'person'
            elif ent.label_ == 'VESSEL_LOCATION':
                entity_type = 'location_on_board'
            elif ent.label_ == 'VESSEL_EQUIPMENT':
                entity_type = 'equipment'
            elif ent.label_ == 'VESSEL_SYSTEM':
                entity_type = 'system'
            elif ent.label_ == 'MARITIME_DOC':
                entity_type = 'document_type'
            elif ent.label_ == 'MAINTENANCE_ACTION':
                entity_type = 'action'
            elif ent.label_ == 'OPERATIONAL_STATE':
                entity_type = 'operational_state'
            ...
            # Generic spaCy NER labels (fallback for base model)
            ...
            # Skip if already extracted
            if ent.text.lower() in already_extracted:
                continue
            ...
            entity = Entity(
                text=ent.text,
                entity_type=entity_type,
                confidence=0.85,  # Good confidence for NER
                source='spacy',
                span=span,
                metadata={'source_file': 'spacy', 'method': 'ner'}
            )

            entities.append(entity)
            spans.append(span)
            already_extracted.add(ent.text.lower())

        # --- Part 2: Noun Extraction ---
        # Extracts generic nouns like "yacht", "crane", "motor", "payment", "activation"
        # Abstract nouns (actions/concepts) vs concrete nouns (equipment/objects)
        abstract_nouns = {
            'activation', 'payment', 'service', 'inspection', 'registration',
            'renewal', 'documentation', 'installation', 'maintenance', 'repair',
            'replacement', 'upgrade', 'calibration', 'certification', 'approval',
            'authorization', 'validation', 'verification', 'completion',
            'shipment', 'delivery', 'procurement', 'purchase', 'order'
        }
        ...
        for token in doc:
            ...
            # Skip single-character nouns
            if len(token.text) <= 1:
                continue

            # Skip if already extracted
            if token.text.lower() in already_extracted:
                continue
            ...
            # Determine entity type based on noun semantics
            if token.text.lower() in abstract_nouns:
                entity_type = 'action'  # Abstract concepts
            else:
                entity_type = 'equipment'  # Concrete objects (yacht, crane, motor, etc.)

            # Create entity
            entity = Entity(
                text=token.text,
                entity_type=entity_type,
                confidence=0.75,  # Moderate confidence for generic nouns
                source='spacy',
                span=span,
                metadata={'source_file': 'spacy', 'method': 'noun'}
            )

            entities.append(entity)
            spans.append(span)
            already_extracted.add(token.text.lower())

        # --- Part 3: Noun Chunks ---
        # Extracts multi-word noun phrases like "invoice number", "purchase order"

        # Question syntax patterns to filter out (avoid extracting request syntax as entities)
        # Note: Patterns exclude articles because spaCy includes them in noun chunks
        question_patterns = [
            ...
            'how many ', 'how much ', 'why is ', 'why are ', 'why was ', 'why were '
        ]

        for chunk in doc.noun_chunks:
            # Skip single-word chunks (already handled by Part 2)
            if len(chunk.text.split()) < 2:
                continue

            # Skip if already extracted
            if chunk.text.lower() in already_extracted:
                continue

            # FILTER: Skip if chunk follows question syntax (e.g., "What is the PO number")
            # Extract text before chunk to check for question patterns
            text_before = text[:chunk.start_char].lower()
            if any(text_before.endswith(pattern) for pattern in question_patterns):
                logger.info(f"[QUESTION FILTER] Skipped '{chunk.text}' - follows question syntax: '{text_before}'")
                continue  # Skip - this is request syntax, not an entity value
            ...
            # Heuristic: If chunk contains abstract nouns, classify as action
            chunk_lower = chunk.text.lower()
            if any(abstract in chunk_lower for abstract in abstract_nouns):
                entity_type = 'action'
            else:
                entity_type = 'equipment'
            ...
            entity = Entity(
                text=chunk.text,
                entity_type=entity_type,
                confidence=0.80,  # Good confidence for noun chunks
                source='spacy',
                span=span,
                metadata={'source_file': 'spacy', 'method': 'noun_chunk'}
            )

            entities.append(entity)
            spans.append(span)
            already_extracted.add(chunk.text.lower())
