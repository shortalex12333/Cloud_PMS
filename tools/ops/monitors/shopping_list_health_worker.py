#!/usr/bin/env python3
"""
shopping_list Health Worker (Render Background Service)
...
FEATURE_FLAGS = [
    "SHOPPING_LIST_LENS_V1_ENABLED",  # e.g., "FAULT_LENS_V1_ENABLED"
    "LENS_SUGGESTIONS_ENABLED",  # e.g., "FAULT_LENS_SUGGESTIONS_ENABLED"
    "LENS_SIGNED_ACTIONS_ENABLED",  # e.g., "FAULT_LENS_SIGNED_ACTIONS_ENABLED"
]
...
def run_health_check() -> Dict[str, Any]:
    ...
    """
    logger.info(f"Starting health check for lens=shopping_list yacht=85fe1119-b04c-41ac-80f1-829d23322598")
    ...
    status, body, latency = check_endpoint(jwt_token, f"/v1/actions/list?domain=shopping_list")
...
def main():
    """Main loop."""
    logger.info(f"Starting shopping_list health worker")
    logger.info(f"Interval: {HEALTH_CHECK_INTERVAL_MINUTES} minutes")
    logger.info(f"API Base: {API_BASE}")
    logger.info(f"Domain: shopping_list")
