...
Generated from: docs/architecture/20_lens_ops/templates/health_worker_template.py
"""
...
def check_feature_flags() -> Tuple[str, Dict[str, str]]:
    ...
    if not RENDER_API_KEY or not RENDER_SERVICE_ID:
        return "unknown", {"error": "RENDER_API_KEY or RENDER_SERVICE_ID not set"}
...
def run_health_check() -> Dict[str, Any]:
    ...
    # Check 2: Feature Flags (optional - may not be configured)
    ...
    # Check 3: List Endpoint (documents domain)
    logger.info("Check 3: List endpoint")
    status, body, latency = check_endpoint(jwt_token, f"/v1/actions/list?domain={DOMAIN}")
    ...
    # Check 4: Execute list_documents action
    ...
    if status >= 500:
        ...
        logger.info(f"✅ Execute list_documents: 200 OK ({latency}ms)")
    else:
    ...
    if result["latencies_ms"]:
        ...
        result["p95_latency_ms"] = sorted_latencies[idx]
    else:
        result["p95_latency_ms"] = None

    total_checks = 2  # list_endpoint + execute_list_documents
    error_checks = sum(1 for c in ["list_endpoint", "execute_list_documents"]
                       if result["checks"].get(c, {}).get("status_code", 0) >= 400)
...
def write_health_check_to_db(result: Dict[str, Any]):
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            check_id = response_data[0]["id"] if response_data else None
...
def main():
    """Main loop."""
    logger.info(f"Starting {LENS_ID} health worker")
