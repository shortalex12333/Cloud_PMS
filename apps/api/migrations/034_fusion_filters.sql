-- Migration 034: Structured Filters for f1_search_fusion
-- ======================================================
-- Adds p_filters jsonb to support lens-specific structured constraints
-- such as status (receiving) and compliance_state (hours_of_rest), and
-- a best-effort item_contains filter for receiving line items.
...
BEGIN
    ...
    RETURN QUERY
    WITH
    -- Helper: structured filter predicate per-row
    -- Applies only when p_filters has relevant keys; otherwise no-op
    -- We evaluate filters inside CTEs by repeating the predicate.
    text_hits AS (
        SELECT
        ...
        WHERE si.yacht_id = p_yacht_id
          ...
          -- Structured filters (lens-specific)
          AND (
            p_filters IS NULL OR (
              -- Receiving.status = {draft|in_progress|rejected|approved}
              (NOT (p_filters ? 'status') OR si.object_type <> 'receiving' OR (si.payload->>'status') = p_filters->>'status')
            )
          )
    ...
    features AS (
        SELECT
            ...
            GREATEST(0, LEAST(1, COALESCE(th.t_rank, 0) / NULLIF(MAX(th.t_rank) OVER (), 0)))::numeric AS s_text_raw,
            ...
            CASE WHEN p_domain IS NOT NULL AND si.object_type = ANY(v_domain_types)
                 THEN LEAST(p_domain_boost, 0.3) ELSE 0.0 END::numeric AS s_domain_raw,
        ...
        WHERE si.yacht_id = p_yacht_id
          -- Structured filters that require payload inspection but weren’t in candidate CTEs
          AND (
            p_filters IS NULL OR (
              -- Hours of rest compliance_state
              (NOT (p_filters ? 'compliance_state') OR si.object_type <> 'hours_of_rest' OR (si.payload->>'compliance_state') = p_filters->>'compliance_state')
            )
          )
          AND (
            p_filters IS NULL OR (
              -- Receiving item_contains best-effort: match payload item_names array or fallback to search_text ILIKE
              (NOT (p_filters ? 'item_contains') OR si.object_type <> 'receiving' OR (
                 EXISTS (
                   SELECT 1 FROM jsonb_array_elements_text(COALESCE(si.payload->'item_names', '[]'::jsonb)) it
                   WHERE it ILIKE ('%' || (p_filters->>'item_contains') || '%')
                 )
                 OR (si.search_text ILIKE ('%' || (p_filters->>'item_contains') || '%'))
              ))
            )
          )
    ),
    ...
    scored AS (
        SELECT
            ...
            f.r_text,
            f.r_vector,
            (p_w_text * f.s_text_raw + p_w_vector * f.s_vector_raw + p_w_recency * f.s_recency_raw)::numeric AS score_base,
            ((1.0 / (p_rrf_k + f.r_text)) + (1.0 / (p_rrf_k + f.r_vector)))::numeric AS rrf_contrib,
            (
              (p_w_text * f.s_text_raw + p_w_vector * f.s_vector_raw + p_w_recency * f.s_recency_raw) +
    ...
    FROM scored s
    ORDER BY s.final_score_calc DESC, s.feat_type ASC, s.feat_id ASC
...
'Deterministic fusion search with domain boost and structured filters.

Supports optional p_filters jsonb, e.g. {"status":"draft"}, {"compliance_state":"violation"},
and {"item_contains":"fuel filter"} (receiving). Filters are yacht-scoped and applied in both
text/vector candidate sets where possible and always at feature stage.';
