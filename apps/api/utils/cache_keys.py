...
def build_cache_key_from_ctx(
    ...
    """
    Build cache key from ActionContext.

    Args:
        ctx: ActionContext from @secure_action decorator
        endpoint: API endpoint name
        query_hash: Hashed query string
        phase: Streaming phase (default 1)
...
def builder_from_ctx(ctx) -> CacheKeyBuilder:
    """
    Create CacheKeyBuilder from ActionContext.

    Args:
        ctx: ActionContext from @secure_action decorator

    Returns:
        CacheKeyBuilder configured with ctx fields
    """
