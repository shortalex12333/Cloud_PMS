...
class PurchasingMutationHandlers:
    ...
    async def add_to_shopping_list_execute(
        self,
        ...
        Tables: shopping_list, audit_log
        ...
        try:
            ...
            # D5: Quantity validation
            quantity = params.get("quantity")
            if not quantity or quantity <= 0:
                builder.set_error("VALIDATION_FAILED", "Quantity must be greater than 0")
                return builder.build()

            # I3: Required fields
            required_fields = ["part_id", "quantity", "reason"]
            ...
            # I4: Validate part exists
            part_result = await self.db.table("parts").select("id, name").eq(
                "id", params["part_id"]
            ...
            await self.db.table("shopping_list").insert({
                ...
                "estimated_unit_cost_usd": params.get("estimated_unit_cost_usd", 0),
                "status": "candidate",  # Initial state
            ...
            await self.db.table("audit_log").insert({
                ...
                "new_values": {
                    "part_id": params["part_id"],
                    "part_name": part_result.data["name"],
                    "quantity": quantity,
                    "status": "candidate"
                },
                "changes_summary": f"User {user['name']} added {quantity}x {part_result.data['name']} to shopping list",
                "risk_level": "low",
                "created_at": created_at
            }).execute()
            ...
            builder.set_message(
                f"Item added to shopping list: {quantity}x {part_result.data['name']}",
                Severity.SUCCESS
            )

        except Exception as e:
            logger.error(f"add_to_shopping_list failed: {e}", exc_info=True)
            builder.set_error("EXECUTION_FAILED", str(e))
...
def get_purchasing_mutation_handlers(supabase_client):
    """Factory function to create purchasing mutation handlers"""
    return PurchasingMutationHandlers(supabase_client)
