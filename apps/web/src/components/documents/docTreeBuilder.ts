...
export function buildDocTree(docs: Doc[]): TreeNode[] {
  ...
  for (const doc of docs) {
    // Use original_path for the user-visible folder hierarchy.
    // storage_path is a blob-storage key — it contains UUIDs and S3 prefixes
    // that have no meaning to the user.
    const rawPath = (doc.original_path || '').trim();
    const allSegments = rawPath ? splitPath(rawPath) : [];
    // Drop the final segment — it's the file itself, not a folder.
    const folderSegments = allSegments.length > 1 ? allSegments.slice(0, -1) : [];
