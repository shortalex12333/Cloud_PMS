/**
 * Microsoft OAuth - Callback Handler (WRITE App)
 *
...
const REDIRECT_URI = `${APP_URL}/api/integrations/outlook/write/callback`;
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.error) {
        ...
        console.error('[Outlook Callback WRITE] Exchange error:', result.error);
      }
    }
