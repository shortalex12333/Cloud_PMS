#!/usr/bin/env python3
"""
{LENS_ID} Signed Flow Acceptance Tests
...
LENS_ID = "{LENS_ID}"  # e.g., "faults"
DOMAIN = "{DOMAIN}"  # e.g., "faults"
SIGNED_ACTION = "{SIGNED_ACTION}"  # e.g., "create_work_order_from_fault"
ENTITY_TYPE = "{ENTITY_TYPE}"  # e.g., "work_order"

# Test data (replace with actual IDs from staging DB)
YACHT_ID = "{YACHT_ID}"  # e.g., "85fe1119-b04c-41ac-80f1-829d23322598"
TEST_ENTITY_ID = "{TEST_ENTITY_ID}"  # e.g., fault_id for work order creation

# Test users (replace with actual user IDs)
USERS = {
    "HOD": {"id": "{HOD_USER_ID}", "email": "hod.test@alex-short.com", "role": "chief_engineer"},
    "CREW": {"id": "{CREW_USER_ID}", "email": "crew.test@alex-short.com", "role": "crew"},
    "CAPTAIN": {"id": "{CAPTAIN_USER_ID}", "email": "captain.test@alex-short.com", "role": "captain"}
}
...
def main():
    ...
    payload = {
        ...
        "payload": {
            "yacht_id": YACHT_ID,
            "{ENTITY_ID_KEY}": TEST_ENTITY_ID,  # e.g., "fault_id": test_fault_id
    ...
    if status == 200:
        ...
        entity_id = None
        if body.get('result'):
            entity_id = body['result'].get(f'{ENTITY_TYPE}_id') or body['result'].get(ENTITY_TYPE, {}).get('id')

        if entity_id:
            record("Test 4: CAPTAIN signature", True, f"200 + {ENTITY_TYPE} created: {entity_id}")
        else:
            record("Test 4: CAPTAIN signature", False, f"200 but no {ENTITY_TYPE}_id in response")
    else:
    ...
    if status == 200:
        ...
        if entity_id:
            record("Test 5: HOD signature", True, f"200 + {ENTITY_TYPE} created: {entity_id}")
        else:
            record("Test 5: HOD signature", False, f"200 but no {ENTITY_TYPE}_id in response")
    else:
        detail = body.get('detail', {}) if isinstance(body, dict) else {}
        error_msg = detail.get('message', '') if isinstance(detail, dict) else str(body)
