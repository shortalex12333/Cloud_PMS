#!/usr/bin/env python3
"""
shopping_list Signed Flow Acceptance Tests
...
SIGNED_ACTION = "create_work_order_from_fault"  # e.g., "create_work_order_from_fault"
ENTITY_TYPE = "shopping_list_item"  # e.g., "work_order"
...
def main():
    ...
    payload = {
        ...
        "payload": {
            "yacht_id": YACHT_ID,
            "item_id": TEST_ENTITY_ID,  # e.g., "fault_id": test_fault_id
    ...
    if status == 200:
        ...
        entity_id = None
        if body.get('result'):
            entity_id = body['result'].get(f'shopping_list_item_id') or body['result'].get(ENTITY_TYPE, {}).get('id')

        if entity_id:
            record("Test 4: CAPTAIN signature", True, f"200 + shopping_list_item created: {entity_id}")
        else:
            record("Test 4: CAPTAIN signature", False, f"200 but no shopping_list_item_id in response")
    else:
    ...
    if status == 200:
        ...
        if entity_id:
            record("Test 5: HOD signature", True, f"200 + shopping_list_item created: {entity_id}")
        else:
            record("Test 5: HOD signature", False, f"200 but no shopping_list_item_id in response")
    else:
        detail = body.get('detail', {}) if isinstance(body, dict) else {}
        error_msg = detail.get('message', '') if isinstance(detail, dict) else str(body)
