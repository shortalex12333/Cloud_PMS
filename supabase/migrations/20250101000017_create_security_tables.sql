-- ============================================================================
-- Security Tables: yachts, user_profiles, user_roles, api_tokens, yacht_signatures
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_yachts_name ON public.yachts(name);

-- USER PROFILES TABLE
CREATE TABLE IF NOT EXISTS public.user_profiles (
    id UUID PRIMARY KEY,  -- References auth.users(id) but no FK
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_active ON public.user_profiles(yacht_id) WHERE is_active = true;
...
-- Add unique constraint (with DO block to handle already exists)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'unique_active_user_yacht_role'
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_user_roles_active_lookup ON public.user_roles(user_id, yacht_id)
...
CREATE INDEX IF NOT EXISTS idx_api_tokens_valid ON public.api_tokens(user_id, yacht_id)
...
CREATE INDEX IF NOT EXISTS idx_yacht_signatures_active ON public.yacht_signatures(yacht_id)
...
BEGIN
    ...
    RAISE NOTICE '✓ Migration 017 Complete - Created % security tables with indexes', table_count;
END $$;
