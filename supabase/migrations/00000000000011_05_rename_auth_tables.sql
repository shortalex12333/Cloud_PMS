...
-- ================================================================================
-- STEP 6: Update Auth Sync Trigger Functions (CRITICAL)
-- ================================================================================
-- These triggers were created in 02_auth_sync_trigger.sql with old table names
-- Must update them to use renamed tables

-- Update handle_new_user() to use auth_users_profiles
...
BEGIN
    ...
    INSERT INTO public.auth_users_profiles (id, yacht_id, email, name)
    VALUES (
        NEW.id,
...
COMMENT ON FUNCTION public.handle_new_user IS 'Auto-create auth_users_profiles when auth.users record is created';

-- Update handle_new_user_role() to use auth_users_roles
CREATE OR REPLACE FUNCTION public.handle_new_user_role()
RETURNS TRIGGER AS $$
BEGIN
    -- Assign default role if metadata specifies it
    IF NEW.raw_user_meta_data->>'role' IS NOT NULL THEN
        INSERT INTO public.auth_users_roles (user_id, yacht_id, role, assigned_by)
        VALUES (
            NEW.id,
            COALESCE(
                (NEW.raw_user_meta_data->>'yacht_id')::UUID,
...
COMMENT ON FUNCTION public.handle_new_user_role IS 'Auto-assign role from signup metadata to auth_users_roles';
...
-- ================================================================================
-- STEP 8: Create get_user_yacht_id helper function
-- ================================================================================
...
-- ================================================================================
-- STEP 9: Fix Document RPC Function
-- ================================================================================
