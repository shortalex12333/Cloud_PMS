...
BEGIN
    ...
    -- Note: auth_users_profiles columns: id, yacht_id, email, name, is_active, created_at, updated_at
    INSERT INTO public.auth_users_profiles (id, yacht_id, email, name, is_active)
    VALUES (
        NEW.id,
        COALESCE(
            (NEW.raw_user_meta_data->>'yacht_id')::UUID,
            (SELECT id FROM public.yachts LIMIT 1)
        ),
        NEW.email,
        COALESCE(
