-- Migration: Filter soft-deleted emails from hybrid search results
-- Adds is_deleted filter to the search_email_hybrid RPC so that
-- soft-deleted messages (moved/archived in Outlook) are excluded.
...
WITH base AS (
    SELECT
        ...
        CASE
            ...
            ELSE 1 - (m.meta_embedding <=> p_embedding) / 2.0
        END AS vector_score
...
Filters out soft-deleted messages (is_deleted = TRUE).
