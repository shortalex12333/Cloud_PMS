...
--
-- TABLES MODIFIED:
--   1. parts - Add inventory tracking with accountability
--   2. work_orders - Add completion tracking with signatures
--
-- TRUST PRINCIPLES:
...
-- =============================================================================
-- 1. ADD INVENTORY TRACKING TO parts
-- =============================================================================

-- Current state: parts has name, part_number, category, manufacturer
...
COMMENT ON COLUMN public.parts.last_counted_at IS
'ACCOUNTABILITY: When was stock last physically counted. Shows freshness of quantity_on_hand.';

COMMENT ON COLUMN public.parts.last_counted_by IS
...
ON public.parts(yacht_id, quantity_on_hand, minimum_quantity)
WHERE quantity_on_hand <= minimum_quantity;

COMMENT ON INDEX idx_parts_low_stock IS
'Fast lookup of low stock items for check_stock_level action and inventory warnings.';

-- =============================================================================
-- 2. ADD COMPLETION TRACKING TO work_orders
-- =============================================================================

-- Current state: work_orders has created_by, updated_by
-- Missing: fault_id (link to trigger), completed_by (accountability!), assigned_to

ALTER TABLE public.work_orders
ADD COLUMN IF NOT EXISTS fault_id UUID REFERENCES public.faults(id) ON DELETE SET NULL,
...
-- Trust-focused comments
COMMENT ON COLUMN public.work_orders.fault_id IS
...
COMMENT ON COLUMN public.work_orders.completion_notes IS
'TRANSPARENCY: WHAT was done. Required field for mark_work_order_complete action. Min 10 characters. Shows work performed.';

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_work_orders_fault
ON public.work_orders(fault_id)
WHERE fault_id IS NOT NULL;

COMMENT ON INDEX idx_work_orders_fault IS
...
COMMENT ON INDEX idx_work_orders_assigned IS
'Fast lookup of active work orders assigned to user (for "my work orders" view).';

CREATE INDEX IF NOT EXISTS idx_work_orders_completed_by
ON public.work_orders(completed_by, completed_at DESC)
WHERE completed_by IS NOT NULL;

COMMENT ON INDEX idx_work_orders_completed_by IS
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Failed to add quantity_on_hand to parts';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Failed to add last_counted_by to parts';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Failed to add completed_by to work_orders';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Failed to add fault_id to work_orders';
    END IF;
